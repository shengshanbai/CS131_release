...
def harris_corners(img, window_size=3, k=0.04):
    ...
    ### YOUR CODE HERE
    dx_squared = convolve(dx ** 2,window)
    dy_squared = convolve(dy ** 2,window)
    dx_dy_multiplied = convolve(dx * dy,window)
    for i in range(H):
        for j in range(W):
            matrix = np.array([[dx_squared[i,j], dx_dy_multiplied[i,j]],[dx_dy_multiplied[i,j], dy_squared[i,j]]])
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    ### YOUR CODE HERE
    sortIndex=dists.argsort(axis=1)
    for i in range(sortIndex.shape[0]):
        smallest=dists[i,sortIndex[i,0]]
        small=dists[i,sortIndex[i,1]]
        if smallest/small<threshold :
            matches.append([i,sortIndex[i,0]])
